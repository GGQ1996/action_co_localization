...
def sep_loss(atten_set, fea_set, device):
    ...
    for i in range(0, len(atten_set), 2):
        ...
        sim_loss = sim_loss + 1.0 / 2 * torch.sum(torch.max(d1 - d5 + 0.5, torch.FloatTensor([0.]).to(device)))
        sim_loss = sim_loss + 1.0 / 2 * torch.sum(torch.max(d1 - d6 + 0.5, torch.FloatTensor([0.]).to(device)))
        sim_loss = sim_loss + torch.sum(torch.max(d1 - 0.001, torch.FloatTensor([0.]).to(device)))
        n_tmp += 1
