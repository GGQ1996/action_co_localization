...
class ANETproposal(object):
    ...
    def __init__(self, ground_truth_filename=None, proposal_filename=None,
                 ...
                 check_status=False, thres_low=0,
                 thres_high=100000):
        ...
        self.ground_truth, self.activity_index = self._import_ground_truth(
            ground_truth_filename, self.thres_low, self.thres_high)
        ...
        if self.verbose:
            ...
            start = self.proposal['t-start'].values[:]
            end = self.proposal['t-end'].values[:]
            cnt = 0
            print("low %d high %d" %(self.thres_low, self.thres_high))
            for i in range(len(start)):
                tmp_len = end[i] - start[i]
                # print(tmp_len)
                if tmp_len > self.thres_low and tmp_len < self.thres_high:
                    cnt += 1
            print '\tNumber of proposals: {}'.format(cnt)
            # print '\tFixed threshold for tiou score: {}'.format(self.tiou_thresholds)

    # change the loaded gt can be used to evaluate different length action
    def _import_ground_truth(self, ground_truth_filename, thres_low, thres_high):
        ...
        for videoid, v in data['database'].iteritems():
            ...
            for ann in v['annotations']:
                if ann['label'] not in activity_index:
                    ...
                    cidx += 1
                action_len = ann['segment'][1] - ann['segment'][0]
                if action_len > thres_low and action_len < thres_high:
