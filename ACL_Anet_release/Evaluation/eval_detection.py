...
class ANETdetection(object):
        ...
        if self.check_status:
            ...
            with open("useless.txt", "r") as f:
                for l in f.readlines():
                    self.blocked_videos.append(l.strip())
        # Import ground truth and predictions.
        video_ids = self._get_videoids(prediction_filename)
        self.ground_truth, self.activity_index = self._import_ground_truth(
            ground_truth_filename, video_ids)
    ...
    def _import_ground_truth(self, ground_truth_filename, video_ids=[]):
    ...
    def _get_videoids(self, prediction_filename):
        with open(prediction_filename, 'r') as fobj:
            data = json.load(fobj)
        # Checking format...
        if not all([field in data.keys() for field in self.pred_fields]):
    ...
    def evaluate(self):
        ...
        for cls, res in enumerate(self.mAP):
            print ("AP at IOU={0:3} is {1:5}".format(cls*0.05 + 0.5, res))
        if self.verbose:
